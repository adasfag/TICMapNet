...
class CustomNuScenesMapDatasetFusion(BaseDataset):
    """
    ...
    def prepare_data(self, idx) -> Any:
        ...
        #时序信息:
        prev_indexs_list = list(range(idx-self.queue_length, idx))#根据queue_length 去 取
        random.shuffle(prev_indexs_list)
        prev_indexs_list = sorted(prev_indexs_list[1:], reverse=True)#按照时序顺序排列
        
        if self.test_mode:
            prev_indexs_list = list(range(idx-(self.queue_length-1), idx))
            prev_indexs_list = sorted(prev_indexs_list, reverse=True)#-1
        ...
        for i in prev_indexs_list:
            i = max(0, i)#负数设置为0
            if data_info['data_samples'].prev =='':#如果当前帧或者当前帧的某个之前帧没有之前帧，用当前帧或者当前帧的某个之前帧替代
                i=pre_i
            ...
            pre_i=i
            data_info=self.pipeline(data_info)
            data_queue.insert(0, data_info)#往前插
            
        
        data_queue = self.union(data_queue)#data_samples inputs 两个批次
        return data_queue
    ...
    def union(self, queue):
        ...
        """
        data = dict()
        data['data_samples'] = [a['data_samples'] for a in queue]#这是一个list
        # inputs = dict()
        ...
        data['inputs'] = torch.stack([a['inputs'] for a in queue])#这是一个数据
        return data
