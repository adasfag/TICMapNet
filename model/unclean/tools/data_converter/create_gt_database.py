...
from mmdet3d.datasets.transforms import LoadPointsFromFile,LoadPointsFromMultiSweeps,LoadAnnotations3D

from datasets.muti_tasks.nus_dataset import CustomNuScenesDataset
...
def create_groundtruth_database(
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        ...
        points = example["points"].tensor.numpy()#260240 5
        gt_boxes_3d = annos["gt_bboxes_3d"].tensor.numpy()#标注
        ...
        point_indices = box_np_ops.points_in_rbbox(points, gt_boxes_3d)#点云是否在bbox内
        ...
        for i in range(num_obj):#遍历每个注释
            filename = f"{image_idx}_{names[i]}_{i}.bin"
            abs_filepath = osp.join(database_save_path, filename)
            rel_filepath = osp.join(f"{info_prefix}_gt_database", filename)

            # save point clouds and image patches for each object
            gt_points = points[point_indices[:, i]]#拿到在某个范围内的点
            gt_points[:, :3] -= gt_boxes_3d[i, :3]#点云坐标标准化这
            ...
            with open(abs_filepath, "w") as f:
                gt_points.tofile(f)#将过滤后的点云文件进行保存

            if (used_classes is None) or names[i] in used_classes:
                db_info = {
                    "name": names[i],#类名字
                    "path": rel_filepath,#相对点云路径
                    "image_idx": image_idx,#图像id
                    "gt_idx": i,#物体id
                    "box3d_lidar": gt_boxes_3d[i],#真值标注
                    "num_points_in_gt": gt_points.shape[0],#点云数量
                    "difficulty": difficulty[i],#标注质量
                }
