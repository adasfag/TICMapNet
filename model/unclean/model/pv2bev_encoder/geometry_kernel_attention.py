...
class GeometrySptialCrossAttention(BaseModule):
    ...
    def forward(self,
        ...
        queries_rebatch = query.new_zeros(
            [bs, self.num_cams, max_len, self.embed_dims])# 4 6 7165 256
        reference_points_rebatch = reference_points_cam.new_zeros(
            [bs, self.num_cams, max_len, D, 2])#4 6 7165 4 2

        for j in range(bs):#每个批次
            for i, reference_points_per_img in enumerate(reference_points_cam):#每个相机
                index_query_per_img = indexes[i]
                queries_rebatch[j, i, :len(
                    index_query_per_img)] = query[j, index_query_per_img]#4 6 7165 256
                reference_points_rebatch[j, i, :len(
                    index_query_per_img)] = reference_points_per_img[j, index_query_per_img]#4 6 7165 4 2
        ...
        queries = self.attention(query=queries_rebatch.view(bs*self.num_cams, max_len, self.embed_dims), key=key, value=value,
                                 ...
                                 level_start_index=level_start_index).view(bs, self.num_cams, max_len, self.embed_dims)#4 6 7165 256
        for j in range(bs):
            for i, index_query_per_img in enumerate(indexes):
                slots[j, index_query_per_img] += queries[j,
                                                         i, :len(index_query_per_img)]# 4 20000 256
