...
class CustomOpenlaneLaneDataset(BaseDataset):
    """
    ...
    def parse_data_info(self, raw_data_info: dict) -> Union[dict, List[dict]]:
        # 格式转换
        lidar2ego = np.eye(4)
        lidar2ego[:3, :3] = Quaternion([0,0,0,1]).rotation_matrix
        lidar2ego[:3, 3] = [0,0,1]
        ...
        for cam_type, cam_info in raw_data_info['cams'].items():
            ...
            cam_extrinsics = cam_info['cam_extrinsics']
            ...
            lidar2img_rt = np.matmul(view_pad, np.linalg.inv(cam_extrinsics))
        ...
        raw_data_info['lidar2img'] = lidar2img_rts   #lidar2img = np.matmul(open_instrinsic, np.linalg.inv(cam_extrinsics))
        ...
        lidar2ego = np.eye(4)
        ego2global = np.eye(4)
        raw_data_info['lidar2global'] = ego2global
